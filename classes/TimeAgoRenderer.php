...
class TimeAgoRenderer {
  ...
  static function getEnqueuedScripts() {
    ...
    return '
<script id="' . self::$scriptName . '" type="text/plain" target="amp-script">' . self::getScriptContents() . '</script>
    ';
  }

  static function getScriptMeta() {
    return '<meta name="amp-script-src" content="' . self::getScriptHash() . '" />';
  }

  static function getScriptHash() {
    $contents = self::getScriptContents();
  ...
  static function getScriptContents() {
    return 'const content = document.querySelector("time");
...
function time_ago_head_hook() {
  echo TimeAgoRenderer::getScriptMeta();
}
?>
